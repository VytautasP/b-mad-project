...
- [ ] **Task 1: Create Manual Time Entry Form Component** (AC: 1, 2, 3, 10, 12)
  - [ ] Create `manual-time-entry-form.component.ts` in `src/app/features/tasks/components/`
  - [ ] Design form with reactive forms using FormBuilder
  - [ ] Add form fields: hours (number input), minutes (number input), note (textarea), date (Angular Material datepicker)
  - [ ] Implement form validation: total time > 0 and < 24 hours (1440 minutes)
  - [ ] Add quick log buttons (15m, 30m, 1h, 2h) that pre-fill hours/minutes fields
  - [ ] Default date picker to today's date
  - [ ] Make form mobile-responsive using Angular Material responsive utilities
  - [ ] Add "Log Time" button to task detail panel that opens form in modal/dialog

- [ ] **Task 2: Integrate Time Entry API Service** (AC: 4)
  - [ ] Update `time-tracking.service.ts` with `logManualTime(taskId, hours, minutes, note, date)` method
  - [ ] Call POST /api/tasks/:id/timeentries with EntryType=Manual
  - [ ] Convert hours and minutes to total minutes for API call
  - [ ] Handle API errors and display error messages to user
  - [ ] Show success notification after successful time entry creation

- [ ] **Task 3: Display Time Entries in Task Detail Panel** (AC: 5, 6, 9, 11)
  - [ ] Create `time-entry-list.component.ts` in `src/app/features/tasks/components/`
  - [ ] Fetch time entries using GET /api/tasks/:id/timeentries on task detail load
  - [ ] Display time entries in list with: User name, Date, Duration (formatted as "2h 30m"), Note, Type icon
  - [ ] Sort time entries by EntryDate descending (most recent first)
  - [ ] Add type icon to distinguish timer entries (⏱️) from manual entries (✏️)
  - [ ] Calculate and display total logged time prominently (e.g., "Total: 12h 45m")
  - [ ] Implement empty state UI when no time entries exist ("No time logged yet")
  - [ ] Format duration helper function to convert minutes to "Xh Ym" format

- [ ] **Task 4: Implement Time Entry Deletion** (AC: 7, 8)
  - [ ] Add delete button (trash icon) to each time entry row
  - [ ] Show delete button only for entries created by current user
  - [ ] Implement confirmation dialog before deletion using Angular Material Dialog
  - [ ] Call DELETE /api/timeentries/:id when confirmed
  - [ ] Remove time entry from list immediately on successful deletion
  - [ ] Update task's total LoggedMinutes display after deletion
  - [ ] Show success notification after deletion
  - [ ] Handle deletion errors gracefully

- [ ] **Task 5: Unit Testing** (AC: 1-12)
  - [ ] Write unit tests for manual-time-entry-form.component.ts (form validation, quick log buttons)
  - [ ] Write unit tests for time-entry-list.component.ts (display, sorting, formatting)
  - [ ] Write unit tests for time-tracking.service.ts (API calls, error handling)
  - [ ] Test form validation edge cases (0 minutes, 24+ hours, negative values)
  - [ ] Test duration formatting helper function
  - [ ] Mock HttpClient responses for service tests
