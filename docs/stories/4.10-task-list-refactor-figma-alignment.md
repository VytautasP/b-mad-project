...
- [ ] Audit current `task-list` component and map each Figma section to needed changes. (AC: all)
- [ ] Add page heading block with "My Tasks" title + active-task count badge. (AC: 1)
- [ ] Implement quick-filter tab bar with All Tasks / Pending / Done toggle buttons and count badges. (AC: 2)
- [ ] Add right-aligned Filter and Sort action buttons to the tab-bar row. (AC: 3)
- [ ] Restructure table columns: add checkbox column, reorder columns, remove Assignees from desktop default. (AC: 4, 5)
- [ ] Replace status icon+text with colour-coded pill badges with dot indicator. (AC: 6)
- [ ] Update task name cell to two-line layout (title + subtitle). (AC: 7)
- [ ] Restyle priority cell with directional icon and colour-coded label. (AC: 8)
- [ ] Apply monospace font to time-logged column; display `--` for zero time. (AC: 9)
- [ ] Restyle actions column with inset shadow separator and compact overflow control. (AC: 10)
- [ ] Wrap table in card container with 12px radius, border, and shadow. (AC: 11)
- [ ] Replace `mat-paginator` with custom pagination footer (text + Previous/Next buttons). (AC: 12)
- [ ] Verify all existing functionality (CRUD, sort, filter, timer, navigation) still works. (AC: 13)
- [ ] Apply Figma typography tokens (Manrope weights/sizes, Liberation Mono for time). (AC: 14)
- [ ] Validate responsive breakpoint: Figma layout at desktop, card view at mobile. (AC: 15)
- [ ] Update / add unit tests for new layout, tab filtering, checkbox selection, and custom pagination.
