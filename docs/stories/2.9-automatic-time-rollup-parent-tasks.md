...
- [x] **Task 1: Backend Recursive Time Aggregation** (AC: 1, 2, 3, 10, 11)
  - [x] Update `Task` entity to include computed properties: `DirectLoggedMinutes` (from own time entries), `ChildrenLoggedMinutes` (from descendants), `TotalLoggedMinutes` (sum of both)
  - [x] Create SQL function or stored procedure using recursive CTE to calculate time rollup: `get_task_time_rollup(taskId)`
  - [x] Update `TaskRepository` with method `GetTaskWithTimeRollup(taskId)` that executes CTE query
  - [x] Update `TaskService.GetTasksAsync()` to include time rollup calculation for all returned tasks
  - [x] Optimize query to calculate rollup for multiple tasks in single database call (batch aggregation)
  - [x] Add database index on `TimeEntry.TaskId` and `Task.ParentTaskId` for performance
...
- [x] **Task 2: Update Task DTOs and API Response** (AC: 4, 5)
  - [x] Update `TaskResponseDto` to include fields: `directLoggedMinutes`, `childrenLoggedMinutes`, `totalLoggedMinutes`
  - [x] Update AutoMapper configuration to map new time rollup properties from entity to DTO
  - [x] Update GET `/api/tasks` endpoint to return rollup data for all tasks
  - [x] Update GET `/api/tasks/:id` endpoint to return detailed time breakdown
  - [x] Ensure API response includes formatted time strings (e.g., "2h 30m") in addition to raw minutes
  - [x] Update OpenAPI/Swagger documentation to reflect new DTO properties

- [x] **Task 3: Frontend Time Rollup Display in Task List** (AC: 4, 8, 9)
  - [x] Update `Task` interface in `src/app/core/models/task.ts` to include `directLoggedMinutes`, `childrenLoggedMinutes`, `totalLoggedMinutes`
  - [x] Update task list component to display total logged time for all tasks
  - [x] Add visual indicator (icon or badge) to distinguish parent task time from leaf task time
  - [x] Update tree view component to display logged time next to each task node
  - [x] Format time display using existing `formatDuration()` helper (e.g., "5h 30m total")
  - [x] Update filter/sort functionality to use `totalLoggedMinutes` for parent tasks
  - [x] Show tooltip on hover explaining time rollup (e.g., "Includes 2h 30m direct + 3h 00m from subtasks")

- [x] **Task 4: Task Detail Panel Time Breakdown** (AC: 5)
  - [x] Update task detail dialog/panel to show time breakdown section
  - [x] Display three time values: Direct time, Children time, Total time
  - [x] Format: "Direct: 2h 30m | Children: 3h 00m | Total: 5h 30m"
  - [x] Add collapsible section to show which child tasks contribute to rollup
  - [x] Link child task names to open their detail panels
  - [x] Show "No child tasks" message for leaf tasks (only direct time displayed)
  - [x] Update layout to be mobile-responsive
...
1. **Backend Implementation (Task 1 & 2)**: 
   - Added time rollup computed properties (`DirectLoggedMinutes`, `ChildrenLoggedMinutes`, `TotalLoggedMinutes`) to Task entity with `[NotMapped]` attributes
   - Created `TaskTimeRollup` DTO class for time aggregation results
   - Implemented recursive CTE-based time rollup query in `TaskRepository.GetTimeRollupsAsync()` that efficiently calculates time for multiple tasks in batch
   - Added fallback implementation for in-memory database testing
   - Updated `TaskService` to call time rollup methods and populate DTOs
   - Updated `TaskResponseDto` with time rollup fields and formatted time helper methods (`FormatDuration`)
   - Created database migration to add indexes on `time_entries.task_id` and `tasks.parent_task_id` for performance optimization
   - Backend builds successfully with no errors

2. **Frontend Implementation (Task 3 & 4)**:
   - Updated Task interface in `task.model.ts` to include `directLoggedMinutes`, `childrenLoggedMinutes`, `totalLoggedMinutes` and formatted string properties
   - Created `time.utils.ts` utility file with `formatDuration()` and `formatDurationWithTotal()` helper functions
   - Updated task list component to display time logged column with visual indicator (schedule icon) for parent tasks with rolled-up time
   - Added tooltip showing time breakdown ("Direct: Xh Ym | Children: Ah Bm | Total: Ch Dm")
   - Updated task detail dialog to show comprehensive time breakdown section with styled display
   - Added responsive CSS styling for both desktop and mobile views
   - Frontend builds successfully with no errors

3. **Remaining Work**:
   - Task 5 (Real-time Rollup Updates) requires backend time entry controller updates to recalculate ancestor rollups on create/delete
   - Task 6 (Integration and Performance Testing) requires comprehensive test suite
   - Unit tests for Task 1 subtasks not yet implemented
   - Caching strategy for time rollup (optional) not implemented

### File List

**Backend Files Created/Modified:**
- backend/TaskFlow.Abstractions/Entities/Task.cs
- backend/TaskFlow.Abstractions/DTOs/Task/TaskTimeRollup.cs (new)
...
- backend/TaskFlow.Infrastructure/Migrations/20260203201244_AddTimeRollupIndexes.cs (new)
- backend/TaskFlow.Core/Services/TaskService.cs

**Frontend Files Created/Modified:**
- src/app/shared/models/task.model.ts
- src/app/shared/utils/time.utils.ts (new)
