...
- **Bug Fix (2026-01-27)**: Fixed expand/collapse state persistence by traversing tree structure instead of relying on treeControl.dataNodes
- **Bug Fix (2026-01-27)**: Fixed ExpressionChangedAfterItHasBeenCheckedError in drag-drop operations by adding ChangeDetectorRef.markForCheck()
- **Bug Fix (2026-01-27)**: Made drag event handling more robust by checking for _dragRef availability before accessing it
- **Bug Fix (2026-01-27)**: Fixed drag-drop parent assignment by adding cdkDropList to each node and using event.container.data to get drop target
