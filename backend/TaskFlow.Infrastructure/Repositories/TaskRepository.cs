...
public class TaskRepository : ITaskRepository
{
        ...
        CancellationToken ct = default)
    {
        ...
        // Base query: tasks with due dates in range (anchor tasks)
        var query = _context.Tasks
            .AsNoTracking()
            .Include(t => t.TaskAssignments)
                .ThenInclude(ta => ta.User)
            .Where(t => !t.IsDeleted
                && t.DueDate != null
                && t.DueDate >= queryDto.StartDate
                && t.DueDate <= queryDto.EndDate
                && (t.CreatedByUserId == userId || userAssignedTaskIds.Contains(t.Id)));

        // Optional filters (only on anchor tasks)
        if (queryDto.AssigneeId.HasValue)
        {
        ...
        var allTaskIds = new HashSet<Guid>(matchingTasks.Select(t => t.Id));
        var allTasks = new List<TaskEntity>(matchingTasks);

        // Walk UP: recursively fetch all ancestors so we find root parents
        var parentIdsToFetch = matchingTasks
            .Where(t => t.ParentTaskId.HasValue && !allTaskIds.Contains(t.ParentTaskId.Value))
            .Select(t => t.ParentTaskId!.Value)
            .Distinct()
            .ToList();

        while (parentIdsToFetch.Count > 0)
        {
            var parents = await _context.Tasks
                .AsNoTracking()
                .Include(t => t.TaskAssignments)
                    .ThenInclude(ta => ta.User)
                .Where(t => parentIdsToFetch.Contains(t.Id) && !t.IsDeleted)
                .ToListAsync(ct);

            foreach (var p in parents)
            {
                if (allTaskIds.Add(p.Id))
                    allTasks.Add(p);
            }

            parentIdsToFetch = parents
                .Where(p => p.ParentTaskId.HasValue && !allTaskIds.Contains(p.ParentTaskId.Value))
                .Select(p => p.ParentTaskId!.Value)
                .Distinct()
                .ToList();
        }

        // Walk DOWN: from every task in the set, fetch all descendants
        var childSearchIds = allTaskIds.ToList();

        while (childSearchIds.Count > 0)
        {
            var children = await _context.Tasks
                .AsNoTracking()
                .Include(t => t.TaskAssignments)
                ...
                .Where(t => t.ParentTaskId.HasValue
                    && childSearchIds.Contains(t.ParentTaskId!.Value)
                    && !t.IsDeleted)
                .ToListAsync(ct);
